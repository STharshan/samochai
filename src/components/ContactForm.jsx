    ...
    <section className="relative overflow-hidden px-4 py-5">
      <div className="relative z-10 max-w-3xl mx-auto pt-10">
        <div className="bg-gradient-to-br from-[#12392c] to-[#0e2a20] rounded-xl p-6 md:p-8 shadow-2xl">
            ...
            {["name", "email", "phone"].map((field) => (
              <div key={field}>
                <input
                  ...
                  className={`w-full bg-[#0a1f17] border ${errors[field] ? 'border-red-500' : 'border-green-700/50'} text-white rounded-lg p-3 outline-none`}
                />
              </div>
            ))}
          </div>
            ...
            {menuItems.map((m, i) => (
              <div key={m.id} className="bg-[#0a1f17] p-4 rounded-lg">
                ...
                <select
                  ...
                  className="w-full mb-2 bg-[#12392c] text-white rounded-lg p-2 disabled:opacity-50"
                >
          ...
          <button onClick={addMenuItem} className="w-full mt-4 bg-green-700 text-white py-3 rounded-lg">
